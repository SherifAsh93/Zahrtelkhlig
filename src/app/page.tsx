...
export default async function HomePage() {
  ...
  return (
    <div dir="rtl">
      ...
      {categories.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 py-10">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 font-cairo">تسوقي حسب القسم</h2>
      ...
      {/* CTA Banner — elegant dark gold */}
      <div
        className="relative overflow-hidden my-2"
        style={{ background: 'linear-gradient(135deg, #0f0f0f 0%, #1c1c1c 50%, #0f0f0f 100%)' }}
      >
        <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(ellipse at 30% 50%, rgba(212,168,50,0.15) 0%, transparent 60%), radial-gradient(ellipse at 70% 50%, rgba(212,168,50,0.1) 0%, transparent 60%)' }} />
        <div className="relative max-w-7xl mx-auto px-4 py-12 sm:py-16 text-center">
          <p className="text-gold-400 font-cairo text-xs sm:text-sm tracking-widest mb-2">✨ New Collection ✨</p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/products?category=eid" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-3 bg-gold-500 text-black rounded-full font-bold font-cairo hover:bg-gold-400 transition-colors shadow-lg text-sm">
              تسوقي الآن
            </Link>
            <a href="https://web.facebook.com/zahrtelkhlig" target="_blank" rel="noopener noreferrer"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-[#1877F2] rounded-full text-white text-sm font-cairo hover:bg-[#166FE5] transition-colors">
            ...
            <a href="https://www.instagram.com/zahretelkhaleej.c/" target="_blank" rel="noopener noreferrer"
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-full text-white text-sm font-cairo"
      ...
      {featured.length > 0 && (
        <section className="bg-white py-10">
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900 font-cairo">المنتجات المميزة</h2>
                <p className="text-gray-500 text-xs sm:text-sm font-cairo mt-1">أبرز منتجاتنا المختارة بعناية</p>
              </div>
      ...
      {newArrivals.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 py-10">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900 font-cairo">وصل حديثاً</h2>
              <p className="text-gray-500 text-xs sm:text-sm font-cairo mt-1">أحدث المنتجات في مجموعتنا</p>
            </div>
            <Link href="/products" className="flex items-center gap-1 text-brand-600 text-sm font-cairo hover:gap-2 transition-all">
              عرض الكل <ArrowLeft size={14} />
            </Link>
